...
Module contains: the CeiloChunk data class
"""
...
from .data import AbstractChunk
from .errors import AmpycloudError
from .logger import log_func_call
from .metarize import MetarizeMixin
...
class CeiloChunk(AbstractChunk, MetarizeMixin):
    ...
    def __init__(
        ...
        self._slices: Optional[pd.DataFrame] = None
        self._groups: Optional[pd.DataFrame] = None
        self._layers: Optional[pd.DataFrame] = None

    @log_func_call(logger)
    def data_rescaled(
        self,
        dt_mode: Optional[str] = None,
        height_mode: Optional[str] = None,
        dt_kwargs: Optional[dict] = None,
        height_kwargs: Optional[dict] = None,
    ) -> pd.DataFrame:
    ...
    def _get_min_sep_for_height(self, height: float) -> float:
        ...
        min_sep_val_id = np.searchsorted(self.prms["MIN_SEP_LIMS"], height)
    ...
    def find_slices(self) -> None:
        ...
        tmp = self.data_rescaled(
            dt_mode="shift-and-scale",
    ...
    def _merge_close_groups(self) -> None:
        ...
        prelim_groups = self._calculate_sligrolay_base_height("groups", prelim_groups, cids)
        # Ordering by height
        ...
        while len(prelim_groups[lt_min_sep_indexer]) > 0:
            ...
            prelim_groups.iloc[idx - 1, prelim_groups.columns.get_loc("height_base")] = (
    ...
    def _build_slice_bundles(self) -> list:
        """Groups overlapping slices into bundles, marking isolated slices as such along the way.

        Returns:
            list: list of slice-index bundles, each bundle being a list of overlapping slice
            indices.

        """
        # Only called after find_groups() has confirmed that slices were already computed.
        assert self._slices is not None

        # Prepare a list of slices that are overlapping with one another.
        slice_bundles: list[list[int]] = []
        ...
        for ind in range(len(self.slices)):
            ...
            seps_m = [
                m_lim < self.slices.iloc[item]["height_max"] + height_pad * self.slices.iloc[item]["thickness"]
                for item in range(ind)
            ]
            seps_p = [
                p_lim > self.slices.iloc[item]["height_min"] - height_pad * self.slices.iloc[item]["thickness"]
            ...
            close_inds = list(np.arange(0, ind, 1)[seps_m]) + list(np.arange(ind + 1, self.n_slices, 1)[seps_p])
    ...
    def _cluster_slice_bundle(self, grp: list) -> None:
        """Runs an Agglomerative Clustering with Single Linkage on a bundle of overlapping
        slices, and assigns the resulting group ids to self.data.

        Args:
            grp (list): indices of the slices belonging to this bundle.

        """

        # Which ceilometer hits belong to this slice bundle ?
        valids = self.data["slice_id"].isin([self.slices.iloc[ind]["cluster_id"] for ind in grp])
        ...
        grp_height_scale = np.max([np.min(self.prms["GROUPING_PRMS"]["height_scale_range"]), grp_height_scale])
        grp_height_scale = np.min([np.max(self.prms["GROUPING_PRMS"]["height_scale_range"]), grp_height_scale])
    ...
    def find_groups(self) -> None:
        ...
        if self._slices is None:
            raise AmpycloudError("Slicing not yet done. You cannot find groups without " + "finding slices first !")
        ...
        # Prepare a list of slices that are overlapping with one another.
        slice_bundles = self._build_slice_bundles()
        ...
        for grp in slice_bundles:
            self._cluster_slice_bundle(grp)
    ...
    def find_layers(self) -> None:
        ...
        if self._groups is None:
            raise AmpycloudError("Grouping not yet done. You cannot find layers without " + "finding groups first !")
        ...
        for ind in range(len(self.groups)):
            ...
            gro_heights = self.data.loc[
                self.data.loc[:, "group_id"] == self._groups.at[ind, "cluster_id"], "height"
            ].to_numpy()
            ...
            if cond1 or cond2 or cond3:
                ...
                logger.info(
                    "Skipping the layering: <min_okta_to_split [%s] | <30 pts [%s] | 1 value [%s]", cond1, cond2, cond3
                )
            ...
            logger.debug(" Cluster %s has %i components according to GMM.", {self.groups.at[ind, "code"]}, ncomp)
            ...
            if ncomp > 1:
                self.data.loc[self.data.loc[:, "group_id"] == self._groups.at[ind, "cluster_id"], "layer_id"] = (
