...
def set_mplstyle(func: Callable) -> Callable:
    ...
    def inner_deco(*args: Any, **kwargs: Any) -> Callable:
        ...
        # 2) Else, I need a string or I cry ...
            ...
            raise AmpycloudError('dynamic.AMPYCLOUD_PRMS["MPL_STYLE"] type unknown:' + f" {type(spec_style)}")
        # 3) Is that a supported style ?
        elif spec_style not in valid_styles():
            raise AmpycloudError(
                f'dynamic.AMPYCLOUD_PRMS["MPL_STYLE"] {spec_style}' + f" unknown. Should be one of {valid_styles()}."
            )
...
def texify(msg: str) -> str:
    ...
    if usetex:
        ...
        msg_splitted = [item.replace("_", r"\_") if ind % 2 == 0 else item for (ind, item) in enumerate(msg.split("$"))]
