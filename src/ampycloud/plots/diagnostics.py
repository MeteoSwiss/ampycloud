...
class DiagnosticPlot:
    ...
    def setup_fig() -> tuple:
        ...
        fig_gs = gridspec.GridSpec(
            ...
            5,
            height_ratios=[1],
            width_ratios=[1, 0.12, 0.3, 0.18, 0.18],
            left=0.08,
            right=0.99,
            bottom=0.15,
            top=0.75,
        ...
        fig.text(
            0.995,
            0.01,
            texify(r"\it\smaller\smaller " + f"Created with ampycloud v{VERSION}"),
            ha="right",
            va="bottom",
            rotation=0,
        )
    ...
    def show_hits_only(self, show_ceilos: bool = False) -> None:
        ...
        if show_ceilos:
            ...
            symb_clrs = np.array(
                [ceilo_clrs[self._chunk.ceilos.index(item) % len(ceilo_clrs)] for item in self._chunk.data["ceilo"]]
            )
        ...
        if show_ceilos:
            ...
            elmts = [
                Line2D([0], [0], ls="", marker="o", color=ceilo_clrs[ind], label=texify(item), markersize=10)
    ...
    def show_slices(self) -> None:
        ...
        self._axs[0].scatter(
            self._chunk.data[self._chunk.data["slice_id"] == -1]["dt"],
            self._chunk.data[self._chunk.data["slice_id"] == -1]["height"],
            marker="s",
            s=15,
            facecolor=fcs[self._chunk.data["slice_id"] == -1],
            edgecolor="k",
        )
        ...
        self._axs[1].text(
            0.5,
            1,
            texify(rf"\smaller $\downarrow$ Slices: {self._chunk.n_slices}"),
            ha="center",
            va="bottom",
        ...
        if self._chunk.n_groups is not None:
            for ind in range(self._chunk.n_groups):
                if show_points:
                    ...
                    in_group = np.array(self._chunk.data["group_id"] == self._chunk.groups.at[ind, "cluster_id"])

                    # I can finally show the points ...
                    self._axs[0].scatter(
                        self._chunk.data[in_group]["dt"],
                ...
                msg = (
                    ...
                    + " "
                    + self._chunk.groups.iloc[ind]["code"]
                    + warn
                )
                self._axs[2].text(
                    0.5,
                    self._chunk.groups.iloc[ind]["height_base"],
                    texify(msg),
                    va="center",
                    ha="center",
                    color="gray",
                    bbox={"facecolor": "none", "edgecolor": "gray", "alpha": alpha, "ls": "--"},
                )
    ...
    def show_layers(self) -> None:
        ...
        self._axs[3].text(
            0.5,
            1,
            texify(rf"\smaller $\downarrow$ Layers: {self._chunk.n_layers}"),
            ha="center",
            va="bottom",
        ...
        if self._chunk.n_layers is not None:
            for ind in range(self._chunk.n_layers):
                ...
                in_layer = np.array(self._chunk.data["layer_id"] == self._chunk.layers.at[ind, "cluster_id"])

                # I can finally show the points ...
                self._axs[0].scatter(
                    self._chunk.data[in_layer]["dt"],
                ...
                self._axs[0].axhline(
                    self._chunk.layers.iloc[ind]["height_base"], xmax=1, c="k", lw=1, zorder=0, ls=lls, clip_on=False
                )
                ...
                self._axs[3].text(
                    0.5,
                    self._chunk.layers.iloc[ind]["height_base"],
                    texify(msg),
                    va="center",
                    ha="center",
                    color="k",
    ...
    def add_vv_legend(self) -> None:
        ...
        self._axs[0].text(
            ...
            1.35,
            texify(msg),
            transform=self._axs[0].transAxes,
            ha="right",
            va="top",
            c="k",
    ...
    def add_ceilo_count(self) -> None:
        ...
        self._axs[0].text(-0.14, -0.14, texify(msg), transform=self._axs[0].transAxes, ha="left")
    ...
    def add_max_hits(self) -> None:
        ...
        self._axs[0].text(-0.14, -0.21, texify(msg), transform=self._axs[0].transAxes, ha="left")
    ...
    def add_metar(self) -> None:
        ...
        self._axs[2].text(
            ...
            ha="center",
            va="top",
            bbox={"facecolor": "none", "edgecolor": "k", "alpha": 1, "boxstyle": "round, pad=0.4"},
        )
    ...
    def format_slice_axes(self) -> None:
        ...
        if self._chunk.n_slices is not None and self._chunk.n_slices > 0:
            ...
            (dt_scale_kwargs, dt_descale_kwargs) = get_scaling_kwargs(
                self._chunk.data["dt"].values,
                "shift-and-scale",
                {"scale": self._chunk.prms["SLICING_PRMS"]["dt_scale"]},
            )

            (height_scale_kwargs, height_descale_kwargs) = get_scaling_kwargs(
                self._chunk.data["height"].values,
            ...
            secax_x = self._axs[0].secondary_xaxis(
                1.06,
                functions=(
                    partial(scaler.apply_scaling, fct="shift-and-scale", **dt_scale_kwargs),
