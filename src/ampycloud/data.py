...
class AbstractChunk(ABC):
    ...
    def _cleanup_pdf(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.msa is not None:
            ...
            data.loc[data[(data.height > hit_height_lim) & (data.type <= 1)].index, 'type'] = 0
            data.loc[data[(data.height > hit_height_lim) & (data.type <= 1)].index, 'height'] = \
                np.nan
            # Type 2 or more hits get cropped (there should be only 1 non-detection per time-stamp).
            data = data.drop(data[(data.height > hit_height_lim) & (data.type > 1)].index)
