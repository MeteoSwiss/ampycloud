...
def flat_layer(dts: np.ndarray, height: float, height_std: float, sky_cov_frac: float) -> DataFrame:
...
def sin_layer(
    ...
    out.loc[:, "height"] = out.loc[:, "height"] + np.sin(-np.pi / 2 + out["dt"] / period * 2 * np.pi) * amplitude
...
def canonical_demo_data() -> DataFrame:
    ...
    lyrs = [
        {"height": 1000, "height_std": 100, "sky_cov_frac": 0.1, "period": 10, "amplitude": 0},
        {"height": 2000, "height_std": 100, "sky_cov_frac": 0.5, "period": 10, "amplitude": 0},
        {"height": 5000, "height_std": 200, "sky_cov_frac": 1, "period": 1800, "amplitude": 1000},
    ]
