...
def test_ceilochunk_init():
    ...
    mock_data = mocker.mock_layers(
        ...
        lookback_time,
        rate,
        [{"height": 1000, "height_std": 10, "sky_cov_frac": 1, "period": 100, "amplitude": 0}],
    )
...
@mark.parametrize(
    "height1,height2,height3,ngroups_expected",
    [
        param(250.0, 800.0, 1200.0, 3, id="gt min sep"),
        param(250.0, 500.0, 1200.0, 3, id="eq min sep"),
        param(250.0, 350.0, 1200.0, 2, id="gt min sep"),
        param(250.0, 350.0, 500.0, 2, id="multiple overlaps no second merge"),
        param(250.0, 350.0, 400.0, 1, id="multiple overlaps with second merge"),
    ],
)
def test_group_separation(height1: float, height2: float, height3: float, ngroups_expected: int):
    ...
    ceilo_chunk = CeiloChunk(fake_data, prms={"MIN_SEP_VALS": [250, 1000], "MIN_SEP_LIMS": [10000]})
...
def test_ceilochunk_nocld():
    ...
    mock_data = mocker.mock_layers(
        ...
        lookback_time,
        rate,
        [{"height": 1000, "height_std": 10, "sky_cov_frac": 0, "period": 100, "amplitude": 0}],
    )
...
def test_ceilochunk_2lay():
    ...
    mock_data = mocker.mock_layers(
        ...
        rate,
        [
            {"height": 1000, "height_std": 10, "sky_cov_frac": 0.5, "period": 100, "amplitude": 0},
            {"height": 2000, "height_std": 10, "sky_cov_frac": 0.5, "period": 100, "amplitude": 0},
        ],
    )
...
def test_layering_singlepts():
    ...
    mock_data = pd.DataFrame(
        np.array([["dummy", -1, 2300, 1], ["dummy", -1, 4000, 2], ["dummy", -1, 4500, 3]]),
...
def test_layering_singleval():
    ...
    mock_data = pd.DataFrame(data.T, columns=["ceilo", "dt", "height", "type"])
...
def test_layering_dualeval():
    ...
    mock_data = pd.DataFrame(np.concatenate([data1, data2], axis=1).T, columns=["ceilo", "dt", "height", "type"])
...
def _mock_layer_from_two_ceilos_with_narrow_stds_and_close_hits(n1: int, n2: int):
    dts1 = np.linspace(-900.0, 0.0, n1)
    layer_ceilo1 = mocker.flat_layer(dts1, 310.0, 2.0, 1.0)
    layer_ceilo1["ceilo"] = np.array(["ceilo1"] * len(layer_ceilo1), dtype=str)
    dts2 = np.linspace(-908.0, -8.0, n2)
    layer_ceilo2 = mocker.flat_layer(dts2, 460.0, 2.0, 1.0)
...
def test_localized_base_height_calculation():
    ...
    chunk_filtered = CeiloChunk(mock_layer, prms={"EXCLUDE_FOR_BASE_HEIGHT_CALC": "ceilo2"})
    ...
    assert chunk_filtered.layers.loc[0, "height_base"] < 330.0
