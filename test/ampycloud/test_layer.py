...
def test_best_gmm():
    ...
    assert best_gmm(np.array([3, 1, 2]), mode="delta", min_prob=1.0, delta_mul_gain=1.0) == 1
    assert best_gmm(np.array([3, 1, 2]), mode="prob", min_prob=1.0, delta_mul_gain=1.0) == 1

    # Correctly handle delta_mul_gain
    assert best_gmm(np.array([3, 2, 1.9]), mode="delta", min_prob=1.0, delta_mul_gain=0.75) == 1
...
def test_ncomp_from_gmm():
    ...
    out, _, _ = ncomp_from_gmm(comp1, scores="BIC", rescale_0_to_x=100, min_sep=1, delta_mul_gain=0.95, mode="delta")
    assert out == 1

    out, _, _ = ncomp_from_gmm(
        np.concatenate([comp1, comp2]), scores="BIC", rescale_0_to_x=100, min_sep=1, delta_mul_gain=0.95, mode="delta"
    )
    assert out == 2

    out, _, _ = ncomp_from_gmm(
        np.concatenate([comp1, comp2, comp3]),
        scores="BIC",
        rescale_0_to_x=100,
        min_sep=1,
        delta_mul_gain=0.95,
...
def test_unstable_layers():
    ...
    with open(Path(__file__).parent / "ref_data" / "Geneva_2019.01.10-04.50.00_SCT040-BKN070.csv", "rb") as f:
        data = pd.read_csv(f)
...
def test_identical_pts():
    ...
    data = np.ones(30) * 24270.0

    out, _, _ = ncomp_from_gmm(
        data, scores="BIC", rescale_0_to_x=100, min_sep=200, random_seed=45, delta_mul_gain=0.95, mode="delta"
    )
