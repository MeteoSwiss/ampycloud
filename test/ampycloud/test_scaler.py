...
def test_shift_and_scale():
    ...
    assert np.all(shift_and_scale(np.array([7e5, 7e5 + 1, 7e5 + 2]), scale=10, mode="do") == np.array([-0.2, -0.1, 0]))
    tmp = np.array([1, 12, 3])
    # Check that this is indeed circular
    assert np.all(
        shift_and_scale(shift_and_scale(tmp, scale=11, mode="do"), scale=11, mode="undo", shift=np.nanmax(tmp)) == tmp
    )
...
def test_step_scale():
    ...
    out = step_scale(vals, steps=[3000, 7000, 10000, 14000], scales=[100, 500, 50, 1000, 10], mode="do")
    ...
    deout = step_scale(out, steps=[3000, 7000, 10000, 14000], scales=[100, 500, 50, 1000, 10], mode="undo")
    assert np.all(np.round(deout, 1) == vals)
...
def test_apply_scaling():
    ...
    out = apply_scaling(np.ones(1), fct="step-scale", scales=[10, 100, 1000], mode="do", steps=[10, 20])
    assert out == 0.1
