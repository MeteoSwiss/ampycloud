...
def test_large_dts(mpls):
    ...
    data = pd.DataFrame(
        [["1", 7e5, 1000, 1], ["1", 7e5 + 1, 1001, 1], ["1", 7e5 + 2, 1002, 1]],
    ...
    with warnings.catch_warnings():
        ...
        diagnostic(
            ...
            show_ceilos=False,
            show=False,
            save_stem="pytest_large_dts",
            save_fmts="png",
            ref_metar_origin="Large dts",
            ref_metar="OVC010",
        )
...
def test_direct_prms(mpls):
    ...
    # Let's create some data with only NaNs
    data = pd.DataFrame(
        [["1", 0, 1000, 1], ["1", 60, 1001, 1], ["1", 120, 1002, 1]], columns=["ceilo", "dt", "height", "type"]
    )
    data["ceilo"] = data["ceilo"].astype(pd.StringDtype())
    data["dt"] = data["dt"].astype(float)
    data["height"] = data["height"].astype(float)
    ...
    with warnings.catch_warnings():
        ...
        diagnostic(
            ...
            show_ceilos=False,
            show=False,
            save_stem="pytest_direct_prms",
            save_fmts="png",
            ref_metar_origin="Direct prms",
            ref_metar="OVC010",
        )
...
def test_empty_plot(mpls):
    ...
    # Let's create some data with only NaNs
    data = pd.DataFrame([["1", -100, np.nan, 0], ["1", -99, np.nan, 0]], columns=["ceilo", "dt", "height", "type"])
    data["ceilo"] = data["ceilo"].astype(pd.StringDtype())
    data["dt"] = data["dt"].astype(float)
    data["height"] = data["height"].astype(float)
    data["type"] = data["type"].astype(int)
    ...
    with warnings.catch_warnings():
        ...
        diagnostic(
            ...
            show_ceilos=False,
            show=False,
            save_stem="pytest_empty_plot",
            save_fmts="png",
            ref_metar_origin="Empty data",
            ref_metar="NCD",
        )
